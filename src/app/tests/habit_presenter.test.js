...
describe('HabitPresenter', () => {
  ...
  it('throws an error when the max habits limit is exceeded', () => {
    presenter.add('Eating', update);
    expect(() => presenter.add('Eating', update)) //
  ...
  describe('reset', () => {
    ...
    it('does not create new object when count is 0', () => {
      ...
      // expect(updatedHabits[1]).toEqual(habits[1]); // object의 내용 비교
      expect(updatedHabits[1]).toBe(habits[1]); // object의 주소 비교(새로운 object가 만들어진 경우 테스트 실패)
    });
  });
